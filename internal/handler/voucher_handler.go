...
func (h *VoucherHandler) SellVoucher(c *fiber.Ctx) error {
	tenantID, _ := c.Locals("tenant_id").(string)
	voucherID := c.Params("id")

	var req sellVoucherRequest
	if err := c.BodyParser(&req); err != nil {
		return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{"error": "Format request tidak valid"})
	}

	if req.BuyerPhone == "" {
		return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{"error": "Nomor telepon pembeli wajib diisi"})
	}

	voucher, err := h.voucherService.SellVoucher(c.Context(), tenantID, voucherID, req.BuyerPhone)
	if err != nil {
		if errors.Is(err, service.ErrVoucherNotFound) {
			return c.Status(fiber.StatusNotFound).JSON(fiber.Map{"error": err.Error()})
		}
...
func (h *VoucherHandler) ActivateVoucher(c *fiber.Ctx) error {
	...
	voucher, err := h.voucherService.ActivateVoucher(c.Context(), tenantID, voucherID)
	if err != nil {
		if errors.Is(err, service.ErrVoucherNotFound) {
			return c.Status(fiber.StatusNotFound).JSON(fiber.Map{"error": err.Error()})
		}
...
func (h *VoucherHandler) CountAvailable(c *fiber.Ctx) error {
	tenantID, _ := c.Locals("tenant_id").(string)
	productID := c.Params("id")

	count, err := h.voucherService.CountAvailable(c.Context(), tenantID, productID)
	if err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{"error": "Kesalahan server internal"})
	}

	return c.JSON(fiber.Map{"available": count})
}

func (h *VoucherHandler) ListPayments(c *fiber.Ctx) error {
	tenantID, _ := c.Locals("tenant_id").(string)

	page, _ := strconv.Atoi(c.Query("page", "1"))
	perPage, _ := strconv.Atoi(c.Query("per_page", "20"))

	filter := repository.VoucherPaymentFilter{
		Search:  c.Query("search"),
		Status:  c.Query("status"),
	...
	payments, total, err := h.voucherService.ListPayments(c.Context(), tenantID, filter)
	if err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{"error": "Gagal memuat daftar pembayaran voucher"})
	}
